...
(Más de 1 transformación geométrica se ve modificada)
...
float toffsetpalabracambiau = 0.0f;
float toffsetpalabrav = 0.0f;
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Cabeza 3 - Movimiento lemniscata
		float x3 = (0.2 * cos(tiempo)) / (1 + sin(tiempo) * sin(tiempo));
		float y3 = (0.2 * cos(tiempo) * sin(tiempo)) / (1 + sin(tiempo) * sin(tiempo));
		...
		modelHEAD5 = glm::translate(modelHEAD5, glm::vec3(-2.6f + cos(tiempo) * 0.2f, 0.16f + sin(tiempo) * 0.2f, 2.9f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(modelHEAD5));
		HEAD5.RenderModel();
		...
		model = glm::translate(model, glm::vec3(0.0f, 0.64f, 6.0f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		ARCO.RenderModel();
	...
	/*
		...
		if (tiempoActual - tiempoAnterior >= 0.5f) { // cambia cada 0.5 segundos
